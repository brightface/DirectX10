...
asClip * Converter::ReadClipData(aiAnimation * animation)
{
	...
	for (UINT i = 0; i < animation->mNumChannels; i++)
	{
		...
		for (UINT k = 0; k < keyCount; k++)
		{
			...
			if (fabs((float)aniNode->mRotationKeys[k].mTime - (float)t) <= D3DX_16F_EPSILON)
			{
			...
			if (fabs((float)aniNode->mScalingKeys[k].mTime - (float)t) <= D3DX_16F_EPSILON)
			{
		...
		clip->Duration = max(clip->Duration, aniNodeInfo.Keyframe.size());
