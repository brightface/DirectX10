...
void RtvDemo::Initialize()
{
	...
	quadShader = new Shader(L"39_Rtv.fxo");
	quad = new MeshRender(quadShader, new MeshQuad());
	Transform* transform = quad->AddTransform();
	transform->Scale(20, 20, 20);
	transform->Position(0, 10, -35);
...
void RtvDemo::Render()
{
	...
	quadShader->AsSRV("RenderMap")->SetResource(renderTarget->SRV());
	quad->Render();
