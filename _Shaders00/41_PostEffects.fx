...
float4 PS_Garysclae(VertexOutput input) : SV_Target
{
...
float4 PS_Garysclae2(VertexOutput input) : SV_Target
{
    float3 grayscale = float3(0.26271, 0.6780f, 0.0593f);
    float3 diffuse = DiffuseMap.Sample(LinearSampler, input.Uv).rgb;
...
float4 PS_Sepia(VertexOutput input) : SV_Target
{
    ...
    return float4(mul(diffuse, ColorTosepiaMatirx), 1);
...
float4 PS_Saturation(VertexOutput input) : SV_Target
{
    ...
    //A + (B - A) * t
    //lerp 식. A가 1이고 B가 3이라고 해보자
    //밝은면만 남는다. 1보다 커지면 밝아진다.
...
{   //center 가 자기 야
    ...
    if(average <0.9f)
        return floatr(0, 0, 0, 1);// 검은색으로 칠하겠다.
        discard; //그 픽셀은 파기하잖아. 
    
        return float4(average, average, average, 1.0f);
    
    return center + Sharpness + float4(average, average, average, 0.1f);
}
...
float4 PS_Vignette(VertexOutput input) : SV_Target
{
    ...
    //원 혹은 구다. //실행하면서 값을 움직임녀서 보라. 포스트 이펙트는
...
technique11 T0
{
    ...
    P_VP(P2, VS, PS_Garysclae)
    P_VP(P3, VS, PS_Garysclae2)
    P_VP(P4, VS, PS_Sepia)
    P_VP(P5,VS,PS_Sharpness)
    P_VP(P6,VS, PS_Vignette)
    P_VP(P6, VS, PS_Interace)
    P_VP(P7,VS, RS_Lens)
    P_VP(P8,VS,PS_Wiggle)
}