...
{	//최초에 랜더 스테이트, 블랜드 스테이트, 레스터라이즈 스테이트 - 그거 초기값 저장해놓은것이다. 왜냐면 랜더링할때마다 기본값을 돌려줄거야. 안해주면 랜더 설정한게 계속 이어져. 꼬인다.
...
{	//블롭이 변한것 있으면 체크한다. 드로우 들어가기 전에 아니셜 스테이트블롭 썼잖아 , 스테이드 바꼈다면 기록
	IPass->ComputeStateBlockMask(&StateBlockMask);
	//이거 해야 자동으로 만든거 쉐이더에 밀어줘야한다.
	D3D::GetDC()->IASetInputLayout(InputLayout);
	IPass->Apply(0, D3D::GetDC()); //그리고 패스 어플라이 하고 랜더링에 들어가.
}
