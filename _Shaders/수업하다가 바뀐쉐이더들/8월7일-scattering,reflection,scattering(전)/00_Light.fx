...
void Texture(inout float4 color, Texture2D t, float2 uv, SamplerState samp)
{
    ...
    //샘플링스테이트 samp 넣고
    [flatten]
    if(any(sampling.rgb)) //모두가 true 색이 하나라도 있다면 , 아니면 원본칼라 유지
...
{
    //메터리얼에대한 초기값 설정해주는 함수
    output = MakeMaterial();

    float3 direction = -GlobalLight.Direction;
    float NdotL = dot(direction, normalize(normal)); //Diffuse 계산식
    
    output.Ambient = GlobalLight.Ambient * Material.Ambient; //Ambient 계산식
    float3 E = normalize(ViewPosition() - wPosition); //시야 
    
    [flatten]
    if(NdotL > 0.0f) //닿은 부분만 계산. 아니면 specular 도 없어
    {
        ...
        {
            float3 R = normalize(reflect(-direction, normal)); //reflect 반사식 다루어 줘야한다. -로 해야 방향이 들어간다. -제곱이기 때문에.
            float RdotE = saturate(dot(R, E)); //스펠큘러는 시야가 달라질때마다 바뀌겠지. emmsisive 외곽에 있다.
            //색끼리는 더해줘.
...
void NormalMapping(float2 uv, float3 normal, float3 tangent, SamplerState samp)
{
    ...
    //samping할거야 그럼 rgb값이 나오겠지? 
    [flatten]
    if(any(map.rgb) == false) //rgb안나오면 할필요 없지
        return;
    ...
    //탄젠트(이미지) 공간 생성 - 정규 직교화 그람슈미트 이용한다.
