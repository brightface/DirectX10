...
VertexOutput_Moon VS_Moon(VertexTexture input)
{
    ...
    output.Position = WorldPosition(input.Position);
    output.Position = ViewProjection(output.Position); //원래걸로 돌려줘
    
    //output.Position = mul(output.Position, View);
    //output.Position = mul(output.Position, MoonProjection); /지운다
    //
    output.Uv = input.Uv;
...
float Fade(float t)
{
    // return t * t * (3.0 - 2.0 * t); //기본식
    return t * t * t * (t * (t * 6.0 - 15.0) + 10.0);//가중치식
}

texture2D CloudMap;
float Noise(float2 P) //컬링노이즈  식 
{
...
float4 PS_Cloud(VertexOutput_Cloud input) : SV_Target
{
    ...
    color.rgb *= 2.0f; //좀더 진해진다. 하늘색이 엷어가지고 
