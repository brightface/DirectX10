...
output.Position = mul(ouPosition.ReflectionVeiw) \
...
//반사는 그냥 두면 될것같고 그러면 MeshOutput으로 바뀌면 되지? Define
VertexOutput_Dome VS_Dome_Reflection(VertexTexture input)
{
    VertexOutput_Dome output;
...
//MESHOUT으로 바꿔주고 - MeshOutput
VertexOutput_Moon VS_Moon(VertexTexture input)
{
    //Meshoutput output = (Meshoutput) 0;//구조체 초기화 
    VertexOutput_Moonoutput;
...
//반사는 냅두고
VertexOutput_Moon VS_Moon_Reflection(VertexTexture input)
{
...
//MeshOutput과 MeshOutput 모든출력은 MeshOutput 으로 통합했다.
VertexOutput_Cloud VS_Cloud_Reflection(VertexTexture input)
{
    //MeshOutput output = (MeshOutput) 0; // geometry 쉐이더로 한방에 그리는거 얘처럼 여러개로 그리는게 아니라
    VertexOutput_Cloudoutput;
...
//ouv 쓰는데가 없지 그러면 지워버려 .  MeshOutput