...
void DepthDemo::Initialize()
{
	...
	shader = new Shader(L"46_Depth.fxo");
	shadow = new Shadow(shader, Vector3(0, 0, 0), 65);

	render2D = new Render2D();
	render2D->GetTransform()->Position
		render2D->GetTransform()->Scale(D3D::Width(), D3D::Height(), 1);
	render2D->SRV(shadow->SRV()); //첫번쨰거는 애를 보여줄거라고
	//두번째것은 depthstencil에 SRV 
...
void DepthDemo::Pass(UINT mesh, UINT model, UINT anim)
{
