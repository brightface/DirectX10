	...
	: shader(shader)
{
	...
	//AS 면 이 World를 가지고 찾는다. 따라서 시간이 오래걸려 그래서 처음에만 찾게 하기위해 값을 저장할 변수를 만든다. 그게 ID3DX11EffectMatrixVariable* sWorld, *sView,*sProjection
	sWorld = shader->AsMatrix("World");
	sView = shader->AsMatrix("View");
	sProjection = shader->AsMatrix("Projection");
	//ID3DX11effectShaderResourceView* sDiffuseMap
...
void Mesh::UpdateWorld()
{
	...
	D3DXMatrixRotationYawPitchRoll(&R, rotation.y, rotation.x, rotation.z); //롤피치 월드 
