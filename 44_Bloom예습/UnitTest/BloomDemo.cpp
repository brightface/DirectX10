...
void BloomDemo::Initialize()
{
	...
	renderTarget[4] = new RenderTarget((UINT)width, (UINT)height); //Composite //합치는단계 //랜더링하고 랜더링누적 한것 합치는것 많이해 
	renderTarget[5] = new RenderTarget((UINT)width, (UINT)height); //ColorGrading 색공간
...
void BloomDemo::PreRender()
{
	...
	//Luminosity
	{
		renderTarget[1]->PreRender(depthStencil);//여기에다 그럴거지
		postEffect->Pass(1);//1번
		postEffect->SRV(renderTarget[0]->SRV()); //원본이니까 애를 선택
		postEffect->Render();
	}
	...
	//BlurY
	{
		...
		postEffect->Pass(2); //2번이 블러니까.
		postEffect->Render();
	}
	//매 업데이트마다 블러를 계산할거야..? 사실 한번만 해도 되는데??
