	...
	: shader(shader)
{
	...
	//computeShader
	{
		//sComputeFrameBuffer = shaer->AsCons
		computeShader = new Shaer (L"26")

		computeShader = new Shader(L"25_GetBones.fx");
		sComputeFrameBuffer = shader->AsConstantBuffer("CB_AnimationFrame");
		sComputeBlendBuffer = shader->AsConstantBuffer("CB_BlendFrame");
		sComputeTransformsSRV = shader->AsSRV("TransformsMap");

		computeBoneBuffer = new StructuredBuffer(NULL, sizeof(Matrix), MAX_MODEL_TRANSFORMS);
		sComputeBoneBuffer = computeShader->AsSRV("ComputeBones");

		//아웃풋 텍스처로 만들거야
		

		//Srv 로 한번만 만들고 밀어주면 돼
	}
}
...
void ModelAnimator::Update()
{
	...
	if (texture == NULL)
	{
		...
		if (frameTime > (1.0f / frameRate))
		{
			...
			sComputeBlendBuffer->SetConstnatBuffer(blendBuffer->Buffer());
			sComputeTransformsSRV->SetResource(trans)
		}
		frmaeTime = fmod(frameTime, (1.0f / frameRate));
