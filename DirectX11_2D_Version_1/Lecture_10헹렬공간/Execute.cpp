...
Execute::Execute()
{
	...
	*/
	...
	//Create World View Projection
	{
		...
		//처음은 0 0 0 위치를 볼것이다.at 어디를 바라볼것이냐? 바라볼 대상 전면부냐 , 앞으로 나아가는 방향 내 앞방향 z 축 1,//위쪽을 향하는 방향벡터  y축, upvector/ cf)앞으로 나아가는 벡터forward 벡터, // 마치 누군가 날 바라보는것처럼, 물체를 이동시키는것 ==눈이 이동하는게 아니라,
		D3DXMatrixLookAtLH(&view, &D3DXVECTOR3(0, 0, 0), &D3DXVECTOR3(0, 0, 1), &D3DXVECTOR3(0, 1, 0)); //원근투영 
		D3DXMatrixOrthoLH(&projection, Settings::Get().GetWidth(), Settings::Get().GetHeight(), 0, 1);

		//투영행렬 2가지방식
		//Perspective 원근 : 원근감이 느껴진다.
		//Orthographic 직교 :원근감이 차이가 없다. 2d , ui를 쓴다. 시야공간은 직육면체처럼 보인다(물체 내부)
