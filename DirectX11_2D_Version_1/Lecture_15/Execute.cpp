...
Execute::Execute()
{
	...
	*/
	...
	//Vertex Data
	{
		...
		vertices[0].uv = D3DXVECTOR2(-1.0f, 1.0f);
	...
	//Create Sampler State
	{
		...
		desc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP; //uv가 기준좌표를 넘어갔을때를 어떻게 할것인가?
		desc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP; //clamp는 그비슷한걸로 채운다.
		...
		desc.Filter = D3D11_FILTER_MIN_POINT_MAG_LINEAR_MIP_POINT;/* D3D11_FILTER_MIN_MAG_MIP_LINEAR;*/
...
void Execute::Render()
{
	...
	graphics->Begin();
	{
		...
		graphics->GetDeviceContext()->PSSetShaderResources(0, 1,/* 2*/ &shader_resource /*shader_resource*/);
