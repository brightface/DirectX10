...
Execute::Execute()
{
	...
	*/
	...
	//Create World View Projection
	{
		...
		//World // 상수값조절해서 늘리지 않고 월드 행렬의
		D3DXMATRIX S; //월드의 크기를 조절하는 함수(크기를 늘려준다.)
		D3DXMatrixScaling(&S, 100, 100, 1); //x,y,한개는 1이상 반드시. 종이의 두께이다.
		//D3DXMatrixScaling(&world, 100, 100, 1); 가 아니라
		...
		D3DXMATRIX T; //위치를 이동시키는 함수 x,y 100,100씩
		...
		//자전행렬
		D3DXMATRIX R; //실수형 기본 double 이다. f 붙어야 float 따라서 캐스팅
