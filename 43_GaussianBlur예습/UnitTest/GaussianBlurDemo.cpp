...
void GaussianBlurDemo::PreRender()
{
	...
	//		postEffect->Pass(2);//2번에저장
	//		postEffect->SRV(renderTarget[1]->SRV()); //srv에 먹인다.
	//		postEffect->Render();
	//	}
	//}
	...
	{
		RenderTarget* rtvs[2];
		rtvs[0] = renderTarget[1]; //0번 1번쓰잖아. 그래서 1번 2번쓰려고 다시 mrt 만든다.
		rtvs[1] = renderTarget[2];

		RenderTarget::PreRender(rtvs, 2, depthStencil);

		postEffect->Pass(3);
		postEffect->SRV(renderTarget[0]->SRV()); //0번것 같다 할거니까
		postEffect->Render();
		//이 결과를 다시 0번에다가 랜더링 할거거든
		//composite를 
		...
		//패스는 4번것 쓰면 되고 합쳐주는거니까
		postEffect->Pass(4);
		//가우시안 mrt 배열이니까 setResourceArray 보낸다.
